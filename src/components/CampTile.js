...
export default function CampTile(props) {
    ...
    const toCampData = (e) => {
        setRedirect(`/person/donate/${props.id}`)
    }
    if(redirect){
  ...
  return (
    <div className='tile-container' >
        <div className='org-camp-title' onClick={toCampData}>
            {props.name} 
        </div>
        ...
        <div className='tile-slots'>
            <p className='tile-slots-no'>{`${props.slots_left}/${props.slots_total}`}</p>
            <p className='tile-slots-msg'>slots remaining</p>
        </div>
